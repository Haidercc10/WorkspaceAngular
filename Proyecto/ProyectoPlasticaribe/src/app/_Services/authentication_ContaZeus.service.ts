...
export class authentication_ContaZeus {
  ...
  constructor(private router: Router,
    ...
    this.userSubject = new BehaviorSubject(JSON.parse(localStorage.getItem('user_Conta_Zeus')!));
  ...
  login() {
    ...
    return this.http.post<any>(`${this.rutaZeusContabilidad}/Authentication/login`, datos).pipe(map(user => {
      this.saveInLocal('Token_Conta_Zeus', user.token);
      localStorage.setItem('user_Conta_Zeus', JSON.stringify(user));
