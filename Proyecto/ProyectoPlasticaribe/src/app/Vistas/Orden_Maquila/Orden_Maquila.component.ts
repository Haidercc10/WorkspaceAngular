    ...
    this.srvMovEntradasMP.getEntradasxMaquilas(orden, data.Id_Mp, data.Id_Tinta, data.Id_Bopp == 449 ? 1 : data.Id_Bopp).subscribe(data => {
      if (data.length > 0) {
        data.forEach(mov => {
          ...
          if(detalle.Id) {
            this.srvMovEntradasMP.Put(detalle.Id, detalle).subscribe(data => { 

            }, error => { this.msj.mensajeError(`Error`, `No fue posible actualizar los movimientos de entrada asociados a esta salida!`) })
          }
        });
      }
    });
