      ...
      this.clientsService.GetSedeClientexNitBagPro(datos.nitCliente).subscribe(dataClient => {
        dataClient.forEach(cli => {
          ...
          this.formDatosProduccion.patchValue({
            ...
            'ancho1': ['Empaque', 'Impresion'].includes(this.proceso) ? datos.ancho1_Extrusion : datos.selladoCorte_Ancho,
            'ancho2': ['Empaque', 'Impresion'].includes(this.proceso) ? datos.ancho2_Extrusion : datos.selladoCorte_Largo,
            'ancho3': ['Empaque', 'Impresion'].includes(this.proceso) ? datos.ancho3_Extrusion : datos.selladoCorte_Fuelle,
            'undExtrusion': datos.und_Extrusion.trim(),
            'calibre': datos.calibre_Extrusion,
            'material': datos.material.trim(),
            'anchoProducto': consulta ? ['Empaque', 'Impresion'].includes(this.proceso) ? (datos.ancho1_Extrusion + datos.ancho2_Extrusion + datos.ancho3_Extrusion) : datos.selladoCorte_Ancho : this.nuevoAnchoProducto,
