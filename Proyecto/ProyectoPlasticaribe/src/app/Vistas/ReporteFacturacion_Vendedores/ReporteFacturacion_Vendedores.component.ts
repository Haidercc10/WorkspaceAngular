...
export class ReporteFacturacion_VendedoresComponent implements OnInit {
  ...
  consultarVendedores() {
    if ([1, 60, 6, 103].includes(this.ValidarRol)) this.usuarioService.GetVendedores().subscribe(datos => this.vendedores = datos);
  ...
  consultarFacturacionVendedor() {
    ...
    let vendedor: string = `${this.vendedorSeleccionado}`.padStart(3, '0');

    const validar = this.datosConsultados.filter(
      item => item.Anio == `${this.anioSeleccionado}` && item.Vendedor == vendedor
    );

    if (validar.length > 0) {
      this.msj.mensajeAdvertencia(
        'Advertencia',
        `¡Ya se ha graficado la información del vendedor con el código ${vendedor} en el año ${this.anioSeleccionado}!`
      );
      this.cargando = false;
      return;
    }
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      consultas.push(
        this.zeusService.GetCostoFacturado_Vendedor(
          vendedor,
          mes,
    ...
    forkJoin(consultas).subscribe({
      next: (resultados) => {
        ...
        const info = {
          label: `Año ${this.anioSeleccionado} - Vendedor ${vendedor}`,
          data: resultados, // Enero a Diciembre
          yAxisID: 'y',
          borderColor: color.substring(0, 4),
          backgroundColor: color.substring(0, 4) + "2",
          pointStyle: 'circle',
