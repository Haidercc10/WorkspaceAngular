...
export class DevolucionesCalidadComponent {
  ...
  devolucion : number = 0; //Variable que almacenará la cantidad de materia prima que fue devuelta en una OT
  ...
  cantProdSinMargenUnd : number | null = null;
  cantProdSinMargenKg : number | null = null;
  CantidadMargen : number | null = null;
  cantProdConMargenKg : number | null = null;
  presentacionProducto : string | null = null;
  ...
  //
  getProcess = () => this.svProcess.srvObtenerLista().subscribe(data => this.process = data.sort((a,b) => a.proceso_Nombre.localeCompare(b.proceso_Nombre)));
      ...
      if(this.infoOT.valid) {
        if(this.validateOrderProduction()) {
          ...
          this.devolutions.push({
            ...
            'supervisor_Id' : this.infoOT.value.supervisor,
            'supervisor' : this.infoOT.value.supervisor != null ? this.supervisors.filter(s => s.usua_Id == this.infoOT.value.supervisor)[0].usua_Nombre : '',
            'operator_Id' : this.infoOT.value.operator,
            'operator' : this.infoOT.value.operator != null ? this.operators.filter(s => s.usua_Id == this.infoOT.value.operator)[0].usua_Nombre: '',
  ...
  createRegisters(){
    ...
    this.devolutions.forEach(x => {
      let info : modelDevoluciones_Calidad = {
        ...
        'Usua_Operario' : x.operator_Id,
        'Usua_Supervisor' : x.supervisor_Id
      }
  ...
  quitRoll(data : any){
    ...
    this.msj.mensajeConfirmacion(`Se ha quitado la OT N° ${data.ot} de la tabla.`);
    this.devolutions.splice(index, 1);
    setTimeout(() => { this.load = true; }, 1000);
  }

  getOperariosYSupervisores(data : any) {
    this.load = false;
    if (data.value != null){
      this.supervisors = [];
      this.operators = [];
    
      // El proceso no tiene relacion con AREA pero sus nombres son similares. Enviamos proceso a la api como AREA para asi obtener los operadores y supervisores.
      let proceso = this.process.find(x => x.proceso_Id === data.value)?.proceso_Nombre;
      this.usuarioService.getSupervisoresYOperariosPorArea(proceso).subscribe(data => {
