...
export class OpedidoproductoComponent implements OnInit {
  ...
  CrearPedidoExterno() {
    const camposPedido : any = {
      ...
      SedeCli_Id: this.FormPedidoExternoClientes.get('PedSedeCli_Id')?.value,
      Estado_Id: this.FormPedidoExternoClientes.get('PedEstadoId')?.value,
  ...
  validarFechas(){
    ...
    FechaCreacion = this.FormPedidoExternoClientes.get('PedFecha')?.value;
    FechaEntrega = this.FormPedidoExternoClientes.get('PedFechaEnt')?.value;
