    ...
    this.inventarioZeusService.GetPedidosAgrupados().subscribe(datos_pedidos => {
      for (let i = 0; i < datos_pedidos.length; i++) {
        if (this.ValidarRol == 2){
          if (this.storage_Id == parseInt(datos_pedidos[i].id_Vendedor)) this.llenarDetallesPedidosZeus(datos_pedidos[i]);
        } else if (this.ValidarRol == 1 || this.ValidarRol == 60 || this.ValidarRol == 61) this.llenarDetallesPedidosZeus(datos_pedidos[i]);
      }
    });
  ...
  llenarDetallesPedidosZeus(datos : any){
    for (let i = 0; i < this.ArrayDocumento.length; i++) {
      if (this.ArrayDocumento[i].data.consecutivo == datos.consecutivo) {
        ...
        this.ArrayDocumento[i].Cant_Items += 1;

        if (datos.existencias >= datos.cant_Pendiente) {
          dataPedidos.ExistenciaMayor = true;
          this.ArrayDocumento[i].ExistenciaMayor = true;
          this.ArrayDocumento[i].Cant_Items_ExistenciaMayor += 1;
        }

        this.ArrayDocumento[i].costo_Cant_Pendiente += datos.costo_Cant_Pendiente;
        this.ArrayDocumento[i].costo_Cant_Total += datos.costo_Cant_Total;
        ...
        this.ArrayDocumento[i].push(dataPedidos);
