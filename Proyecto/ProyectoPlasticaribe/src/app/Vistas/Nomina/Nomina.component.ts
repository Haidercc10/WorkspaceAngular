 ...
 //Funcion que leerá la informacion que se almacenará en el storage del navegador
  ...
  exportarExcel(numero : number) {
    ...
    setTimeout(() => {
      ...
      if(numero == 2) {
        for (const item of datos) {
          const datos1  : any = [item.Cedula, item.Operario, item.Fecha, item.Bulto, item.Referencia, item.Nombre_Referencia, item.Cantidad_Total, item.Cantidad, item.Presentacion, item.Maquina, item.Peso, item.Turno, item.Proceso, item.Precio, item.Valor_Total, item.Pesado_Entre ];
          infoDocumento.push(datos1);
        }
      }
