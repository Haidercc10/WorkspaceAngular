...
export class AsignacionTintasComponent implements OnInit {
  ...
  materiasPrimas: any = []; //Varibale que almacenará las materias primas que se utilizan para la creacion de tintas
  unidadMedida: any = []; //Variable que almacenará las unidades de medida
  tintas: any = []; //Varibale que almacenará las tintas que se pueden crear
  ArrayMateriaPrima: any = []; //Varibale que almacenará las materias primas que se estan asignando para crear una tinta
  ...
  obtenerUltimoIdAsignacion() {
    this.asignacionMPxTintas.srvObtenerUltimaAsignacion().subscribe(datos_asignaciones => {
      ...
      this.actualizarMovimientosEntradasMP(datos_asignaciones.asigMPxTinta_Id);
  ...
  actualizarMovimientosEntradasMP(idAsignacion: number) {
    ...
    this.ArrayMateriaPrima.forEach(mp => {
      mp.Cantidad2 = mp.Cant;
