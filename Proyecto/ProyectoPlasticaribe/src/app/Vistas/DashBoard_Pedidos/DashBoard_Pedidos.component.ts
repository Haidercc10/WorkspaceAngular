...
export class DashBoard_PedidosComponent implements OnInit {
  ...
  ngOnInit() {
    this.lecturaStorage();
    if ([1, 60, 61, 6, 12, 96, 85, 97, 2, 98, 10].includes(this.ValidarRol)) this.tiempoExcedido();
    setInterval(() => {
      this.modoSeleccionado = this.AppComponent.temaSeleccionado;
  ...
  pedidosZeus() {
    ...
    if (this.ValidarRol == 2) {
      sales = `?sales=${String(this.storage_Id).padStart(3, '0')}`;
    }
    this.zeusService.getPedidosCliente(sales).subscribe(datos_pedidos => this.pedidosClientes = datos_pedidos); // Pedidos CLientes
    this.zeusService.getPedidosProductos(sales).subscribe(datos_pedidos => this.pedidosProductos = datos_pedidos); // Pedidos Productos
    this.zeusService.getPedidosVendedores(sales).subscribe(datos_pedidos => this.pedidosVendedores = datos_pedidos); //Pedidos Vendedores
    this.consultarPedidosEstados(sales); // Pedidos Estados
    this.consultarPedidosOrdenesTrabajo(sales); // Pedidos con ordenes de trabajo asociadas
    this.zeusService.getPedidosStock(sales).subscribe(datos_pedidos => this.pedidosStock = datos_pedidos); //Pedidos Stock
  ...
  llenarGraficaPedidosVendedores() {
    ...
    let length: number = this.ValidarRol == 2 ? this.pedidosVendedores.length : 5;
