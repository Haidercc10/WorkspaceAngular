...
export class Facturacion_OrdenMaquilaComponent implements OnInit {
    ...
    this.dtOrdenMaquilaService.getInfoOrdenMaquila_Id(id).subscribe(datos_orden => {
      for (let i = 0; i < datos_orden.length; i++) {
        ...
        this.ordenMaquila_facService.GetOrdenMaquilaFacturada(id, mp).subscribe(datos_Facturacuin => {
          ...
          this.mensajeService.mensajeError(`Error`, `¡Ocurrió un error al consultar la orden de Maquila #${id}, verifique que esta existe e intentelo de nuevo!`);
          this.cargando = false;
        });
      }
    }, () => {
  ...
  validarMateriaPrima(data : any){
    this.cargando = true;
    this.materiasPrimas = [];
    this.materiasPrimas.push(data.filter((item) => item.Exits == true), 1);
  ...
  llenarMateriaPrima(item : any){
    this.cargando = true;
    this.materiasPrimas.splice(this.materiasPrimas.findIndex((data) => item.Id == data.Id), 1);
    this.materiaPrimasSeleccionadas.sort((a,b) => Number(a.Id) - Number(b.Id) );
    this.materiaPrimasSeleccionadas.sort((a,b) => Number(a.Exits) - Number(b.Exits) );
    this.calcularPrecio();
  }
  ...
  quitarMateriaPrima(item : any){
    this.cargando = true;
    this.materiaPrimasSeleccionadas.splice(this.materiaPrimasSeleccionadas.findIndex((data) => item.Id == data.Id), 1);
    this.materiasPrimas.sort((a,b) => Number(a.Id) - Number(b.Id) );
    this.materiasPrimas.sort((a,b) => Number(a.Exits) - Number(b.Exits) );
    this.calcularPrecio();
  }
  ...
  calcularPrecio(){
    ...
    for (let i = 0; i < this.materiaPrimasSeleccionadas.filter((item) => !item.Exits).length; i++) {
  ...
  validarDatos(){
    ...
    if (this.formFacturacionOrden.valid) {
      if ((factura != null && remision == null) || (factura == null && remision != null)){
        ...
        else this.mensajeService.mensajeAdvertencia(`Advertencia`, `¡Debe cargar minimo una materia prima a entregar!`);
      } else this.mensajeService.mensajeAdvertencia(`Advertencia`, `¡Debe llenar el campo "Nro. Factura" o el campo "Nro. Remisión" para crear la facturación!`);
  ...
  crearFacturacionMaquila() {
    ...
    this.facturacion_OMService.insert(info).subscribe(datos => {
      ...
      this.mensajeService.mensajeError(`Error`, `¡No se pudo guardar la facturación!`);
      this.cargando = false;
    });
  }
  ...
  crearDtFacturacionMaquila(id : number){
    for (let i = 0; i < this.materiaPrimasSeleccionadas.length; i++) {
      ...
      this.dtFacturacion_OMService.insert(info).subscribe(() => { }, () => {
        this.mensajeService.mensajeError(`Error`, `¡Ocurrió un error al guardar los detalles de las materias primas de la facturación!`);
        this.cargando = false;
      });
    }
  }
  ...
  crearRelacionFacturacionMaquila(fac : number){
    ...
    this.ordenMaquila_facService.insert(info).subscribe(() => {}, () => {
      this.mensajeService.mensajeError(`Error`, `¡No se pudo crear la relación entre la Orden Maquila y la Facturación!`);
      this.cargando = false;
    });
  }
  ...
  cambiarEstadoOrden(factura : number){
      ...
      setTimeout(() => {
        if (!error) {
          for (let i = 0; i < datos_orden.length; i++) {
            ...
            this.ordenMaquilaService.put(om, info).subscribe(() => { }, () => {
              this.mensajeService.mensajeError(`Error`, `¡Ocurrió un error al cambiar el estado de la Orden de Maquila!`);
              this.cargando = false;
            });
            break;
          }
          ...
          this.facturacion_OMService.put(factura, info).subscribe(() => {})
        }
      }, 3000);
    }, () => {
      error = true;
    ...
    setTimeout(() => {
      this.cargando = false;
      this.mostrarEleccion(factura, `Advertencia`, `¡Se realizó la facturación de la orden de maquila #${om}!, ¿desea ver el detalle en pdf?`);
    }, 5000);
  }
  ...
  onReject = () => this.messageService.clear('pdf');
